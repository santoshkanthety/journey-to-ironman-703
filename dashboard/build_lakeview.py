...
DATASETS = [
    ("kpis", f"""
        SELECT l.ctl, l.atl, l.tsb,
               ...
               (SELECT ROUND(SUM(km),0) FROM {T}.activities WHERE sport='run')  AS run_km,
               (SELECT ROUND(SUM(km),0) FROM {T}.activities WHERE sport='bike') AS bike_km
    ...
    ("monthly", f"SELECT month, sport, hours, km, sessions, tss FROM {T}.monthly_volume WHERE month >= date_sub(current_date(), 1830) ORDER BY month"),
    ("dowhour", f"SELECT dow, dow_name, hour, SUM(sessions) AS sessions FROM {T}.dow_hour GROUP BY dow, dow_name, hour"),
    ("yearly", f"SELECT year, sport, km, hours, sessions, longest_km FROM {T}.yearly_summary ORDER BY year"),
    ("distdist", f"SELECT km_bucket, runs FROM {T}.run_distance_dist ORDER BY km_bucket"),
    ("efforts", f"SELECT band, meters, year, date, est_time, pace_min_km, pace_s_km, run_km, is_all_time_pr FROM {T}.best_efforts ORDER BY meters, year"),
    ("efforts_pr", f"SELECT band, est_time, pace_min_km, date, run_km FROM {T}.best_efforts WHERE is_all_time_pr ORDER BY meters"),
    ("acwr", f"SELECT date, km_7d, km_28d_weekly_avg, acwr FROM {T}.run_load_ratio WHERE date >= date_sub(current_date(), 365) ORDER BY date"),
    ("recent", f"SELECT date, sport, title, ROUND(km,1) AS km, ROUND(hours,2) AS hours, avg_hr, tss FROM {T}.activities ORDER BY start_time DESC LIMIT 100"),
]
...
overview = [
    ...
    counter("c_rkm", "kpis", "run_km", "Run km (all time)", 5, 0),
    line("l_pmc", "pmc", "date",
...
activities = [
    ...
    bar("b_year", "yearly", "year", "km",
        "Distance by year and sport", 3, 8, 3, 8, color="sport"),
    bar("b_dist", "distdist", "km_bucket", "runs",
        "Run distance distribution (km buckets)", 0, 16, 3, 7),
    table("t_recent", "recent",
          ["date", "sport", "title", "km", "hours", "avg_hr", "tss"],
          "Last 100 activities", 3, 16, 3, 7),
]

profile = [
    table("t_pr", "efforts_pr", ["band", "est_time", "pace_min_km", "date", "run_km"],
          "Best efforts — all-time (est. from avg run pace)", 0, 0, 6, 6),
    line("l_eff", "efforts", "year", [("pace_s_km", "Pace (s/km)")],
    ...
    line("l_vol", "acwr", "date", [("km_7d", "7-day km"),
                                   ("km_28d_weekly_avg", "28-day weekly avg km")],
         "Run volume — acute vs chronic", 3, 13, 3, 7),
    table("t_efforts", "efforts",
          ["band", "year", "est_time", "pace_min_km", "date", "run_km"],
